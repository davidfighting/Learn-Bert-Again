...
class InputExample(object):
    ...
    def __init__(self, guid, text_a, text_b=None, label=None):
        ...
        """
        #Using unique identities is to ensure the data is only one and show where the model have trained. 
        #If your csv don't have guid, you can use the module named "uuid" to gen the new one.
        self.guid = guid   
        self.text_a = text_a
        self.text_b = text_b
        self.label = label
...
def main(args):
    ...
    parser.add_argument("--hidden_size", default=1024, type=int,
                    help="Hidden size for the linear layer.")
    ...
    parser.add_argument("--train_steps", default=13000, type=int,
                        help="")
    ...
    # Prepare model
    model = BertForSequenceClassification.from_pretrained(args.model_name_or_path, config=config, args=args)
    ...
    if args.do_train:
        ...
        # 计算预热步数
        num_warmup_steps = int(args.train_steps * 0.1)  # 10% 的训练步数作为预热步数

        # 创建学习率调度器
        scheduler = get_linear_schedule_with_warmup(optimizer, num_warmup_steps,num_training_steps=args.train_steps)

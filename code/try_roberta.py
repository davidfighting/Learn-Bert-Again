...
from __future__ import absolute_import #这个是绝对引用，从系统路径导入的意思，避免用相对路径使得因存在多个文件名导致的引用混乱
...
class InputExample(object):
    ...
    def tokenize_text(self, tokenizer, max_length):
        # Tokenize the text using the provided tokenizer
        inputs = tokenizer(self.text_a, padding='max_length', max_length=max_length, truncation=True, return_tensors="pt")
        return inputs
...
class BertClassifier(nn.Module):
    def __init__(self, bert_path, dropout=0.5, hidden_size=600, gru_input_size=1024, num_layers=1):
        super(BertClassifier, self).__init__()
        self.bert = BertModel.from_pretrained(bert_path)
        self.dropout = nn.Dropout(dropout)
        self.transform = nn.Linear(in_features=gru_input_size, out_features=hidden_size)  # Linear layer transforms BERT's output from gru_input_size to hidden_size
        self.gru = nn.GRU(input_size=hidden_size, hidden_size=hidden_size, num_layers=num_layers)  # Create a GRU model, input size is hidden_size, output size is hidden_size
        self.linear = nn.Linear(hidden_size, 3)  # Change the input size of the output layer to hidden_size, and assume the output size is 3
        self.relu = nn.ReLU()
    ...
    def forward(self, inputs, labels=None):
        ...
        for i in range(inputs['input_ids'].size(1)):
            ...
            attention_mask = inputs['attention_mask'][:, i] if 'attention_mask' in inputs else None
            outputs = self.bert(input_ids=input_ids, token_type_ids=token_type_ids, attention_mask=attention_mask, return_dict=False)
            pooled_output = outputs[1]
            transform_output = self.transform(pooled_output)
            gru_output, _ = self.gru(transform_output.unsqueeze(0))
            dropout_output = self.dropout(gru_output.squeeze(0))
            linear_output = self.linear(dropout_output)
            final_layer = self.relu(linear_output)
            final_outputs.append(final_layer)

        combined_output = torch.mean(torch.stack(final_outputs), dim=0)
        
        if labels is not None:
            loss_fct = nn.CrossEntropyLoss()
            loss = loss_fct(combined_output.view(-1, num_labels), labels.view(-1))
        else:
            loss = torch.tensor(0.0,requires_grad=True)  # return a zero loss if no labels

        return loss, combined_output
...
def convert_examples_to_features(examples, tokenizer, max_seq_length,split_num,
    ...
    #swag means The Situations With Adversarial Generations. Like there is a situation and three choice. Bert should answer each possibilities of the choice.
...
def _truncate_seq_pair(tokens_a, tokens_b, max_length):
    ...
    # This code try to let the content is under the need of the length of bert. If the content is too long, bert can't run well.
    # And it is also a trade-off. Because we may lose some information. But the longer one may not contain too may information of a token. So don't need to give too much worry about it.
...
def accuracy(out, labels):
    #This code means that we choose F1 score as our standard of accuracy.
    #F1 score means that precision and recall are both important!
...
def set_seed(args):
    #This is very important! Actually, I did't care about it before. The reason we should set a seed is that it can let our result can be reproduced.
    #In our first task, we also set a seed. But I didn't know why. And some paper note that seed can affects our result. But we can't set a seed randomly, we should choose like people in this area, and try different seed, and we should also report each result. The most important thing is that we can't "make data".
...
def main(args):
    ...
    parser.add_argument("--eval_steps", default=500, type=int,
                        help="")
    #hidden layers are important layers of the model. You can imagine that many cells in our body learn the features of our sample. Although we can't see, it actually work.
    parser.add_argument("--lstm_hidden_size", default=300, type=int,
                        help="")
    parser.add_argument("--lstm_layers", default=2, type=int,
                        help="")
    #dropout is to let the section of model not to be as same as our sample and prevent it from overfitting.
    parser.add_argument("--lstm_dropout", default=0.5, type=float,
                        help="")    
    #train step is like we read books one by one word or one by one sentence of one by one page. It determines the speed of model to learn the data.
    parser.add_argument("--train_steps", default=10000, type=int,
                        help="")
    #report step is like when bert will report its process.
    parser.add_argument("--report_steps", default=-1, type=int,
                        help="")
    #warmup step is important to training model. It can gradually change learn rate to prevent the model from overfitting. And if we set 0, there will be no warmup.
    ...
    BERT_PATH = args.model_name_or_path
    config = BertConfig.from_pretrained(args.model_name_or_path, num_labels=3)
    
    # Prepare model
    model = BertClassifier(bert_path=args.model_name_or_path)
    ...
    #this code use mixed precision training https://zhuanlan.zhihu.com/p/441591808. It will let our train become fast and efficient.
    if args.fp16:
        model.half()
    model.to(device)
    ...
    if args.do_train:
        ...
        for file in ['dev.csv']:
            ...
            for input_ids, input_mask, segment_ids, label_ids in eval_dataloader:
                    ...
                    with torch.no_grad():
                        inputs = {'input_ids': input_ids, 'token_type_ids': segment_ids, 'attention_mask': input_mask, 'labels': label_ids}
